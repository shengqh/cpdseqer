...
if (!exists('smps')) {
  ...
  contRange <- switch(cntType,
	...
	for (sample in samples) {
		...
		bed <- symRes$bed
		bedS[[sample]] <- bed #bed[bed[,4]%in%dinuc,] # UPDATE 8/19/20
...
knitr::opts_chunk$set(echo = TRUE,fig.width=12,fig.height=6)
plotM_rcDistrib.res <- plotM_rcDistrib(bedS,exp,gn,dinuc,pts=c(0,5,10)) # UPDATE 8/6/20 with added gn argument
write.table(plotM_rcDistrib.res$normFac,'normFactors.txt',quote=F,sep='\t',col.names=F)
