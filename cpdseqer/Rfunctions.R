...
# INPUT libSizeNorm: either NULL or file name for normalization factors generated by cpdseqer size_factor.
...
singleSample <- function(smp,gn,reg=NULL,cntType='rCnt',cnt5file='gn_reg_cnt5.tsv0',libSizeNorm=NULL,smpnames=NULL,nDigit=4,unit2=c(1e6,1e3)) { #nDigit: number of significant digits.
  ...
  if (!is.null(p4gn)) { # Somehow we might fail retrieving expected p-vector
    ...
    names(p5gn) <- c(DINUC4,'others')
		smpCnt <- collapseGrp2Smp(smp,cntType,libSizeNorm,smpnames)
		if (!is.null(unit2)) {
...
twoReg <- function(smp1,smp2,reg1,reg2,gn,cntType='rCnt',cnt5file='gn_reg_cnt5.tsv',libSizeNorm=NULL,smp1names=NULL,smp2names=NULL,nDigit=4,unit2=c(1e6,1e3),DINUC4=c('TT','TC','CC','CT')) {
	smpCnt1 <- collapseGrp2Smp(smp1,cntType,libSizeNorm,smp1names)
	smpCnt2 <- collapseGrp2Smp(smp2,cntType,libSizeNorm,smp2names)
	if (!is.null(unit2)) {
...
twoGrp <- function(smp1,smp2,cntType='rCnt',libSizeNorm=NULL,smp1names=NULL,smp2names=NULL,nDigit=4,DINUC4=c('TT','TC','CC','CT'),unit2=c(1e6,1e3)) {
	smpCnt1 <- collapseGrp2Smp(smp1,cntType,libSizeNorm,smp1names)
	smpCnt2 <- collapseGrp2Smp(smp2,cntType,libSizeNorm,smp2names)
	if (!is.null(unit2)) {
		res <- convert.cntUnit(rbind(smpCnt1,smpCnt2),unit2)
...
# NOTE: when libSizeNorm is not NULL, the function looks for file libSizeNorm where normalization factors are tagged with sample labels.
# INPUT grp: a comma separated string, implying multiple CNT files.
# INPUT cntType: {rCnt, sCnt}
# INPUT libSizeNorm: either NULL or file name for normalization factors generated by cpdseqer size_factor
...
collapseGrp2Smp <- function(grp,cntType='rCnt',libSizeNorm=NULL,smpNames=NULL) {
	samples <- unlist(strsplit(grp,','))
	cnt.samples <- matrix(nr=length(samples),nc=5)
	for (i in 1:length(samples)) {
		sample <- samples[i]
		cnt5 <- collapse.cntFile(sample,cntType)
		cnt.samples[i,] <- cnt5
	}
	# Apply library size normalization with TMM factors
	if (!is.null(libSizeNorm)) { # libSizeNorm is specified as a file name for normalization factors
    normFac0 <- read.delim(libSizeNorm,as.is=T,header=T,row.names=1)
    smpNames <- unlist(strsplit(smpNames,','))
    normFac <- normFac0[smpNames,'SizeFactor']
		rawLibSize <- normFac0[smpNames,'LibrarySize']
    cnt.samples <- cnt.samples/(normFac*rawLibSize/1000000) # normalized CPM with both normFac & rawLibSize are read from pre-generated TXT file 
	}
...
plotM_rcDistrib <- function(bedS0,exp='Experiment',gn='hg38',libSizeNorm=NULL,dinuc=c('TT','TC','CC','CT'),pts=c(0,5,10)) {
	...
	if (!is.null(libSizeNorm)) {
		normFac0 <- read.delim(libSizeNorm,as.is=T,header=T,row.names=1)
		smpNames <- names(bedS0)
		normFac <- normFac0[smpNames,'SizeFactor']
		rawLibSize <- normFac0[smpNames,'LibrarySize']
	} else {
		rawLibSize <- sapply(bedS0,function(x) sum(x[,5]))
		normFac <- rep(1,length(bedS0))
	}
